...
Codexに新しいネイティブツールを追加するものではありません。Codexの`terminal`経由で、ローカルにインストール済みの`hermes` CLIを`-z --toolsets x_search`で実行し、X上の投稿・スレッド・反応を検索します。
...
4. wrapper scriptが`hermes -z --toolsets x_search`をsubprocessで実行する
5. Hermes Agentが自身の設定、認証情報、Python環境で`x_search`を実行する
6. 結果JSONをCodexが読み、要約して回答する

## 必要条件

- Codexが動くローカル環境
- `hermes` CLIが`PATH`上にあること
- Hermes Agent側でCLI platformの`x_search` toolsetが有効になっていること
- xAI認証情報のどちらか
...
```json
{
  ...
  "hermes_command": "/home/user/.local/bin/hermes"
}
```

`requirements_ok`が`false`の場合は、Hermes CLIのCLI platformで`x_search`が無効、または未登録の状態です。
`hermes tools list`で`x_search`が有効か確認してください。実検索時に認証エラーが出る場合は、Hermes側のxAI OAuthまたは`XAI_API_KEY`を確認してください。
...
`hermes` CLIが見つかりません。`hermes`が`PATH`上にあるか確認してください。
別の場所にある場合は`--hermes-command /path/to/hermes`で指定できます。

### `requirements_ok`が`false`

Hermes CLIのCLI platformで`x_search`が無効、または未登録の状態です。

確認するもの:

- `hermes tools list`で`x_search`が有効になっているか
- `hermes auth add xai-oauth`を実行済みか
- `~/.hermes/.env`に`XAI_API_KEY`があるか
- `--check`の`hermes_home`と`hermes_command`が普段のHermes環境と一致しているか
...
Hermes Agent本体はこのリポジトリには含めていません。実行時にローカルの`hermes` CLIを呼び出します。Hermes Agentは、確認時点ではNous ResearchのMIT Licenseで配布されています。公開する場合でも、このリポジトリがHermes Agent本体のコードを再配布していない限り、通常はHermes Agent側の著作権表示を同梱する必要は薄い構成です。
