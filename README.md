...
```json
{
  ...
  "hermes_agent": "/home/user/.hermes/hermes-agent",
  "python_executable": "/home/user/.hermes/hermes-agent/venv/bin/python"
}
```

`requirements_ok`が`false`の場合は、Hermes側のxAI OAuthまたは`XAI_API_KEY`を確認してください。
`credential_source`が空の場合、wrapper scriptから利用可能なxAI認証情報が見えていません。
`hermes_home`、`hermes_agent`、`python_executable`が、普段Hermes Agentを動かしている環境と一致しているかも確認してください。
...
- `--check`の`credential_source`が空になっていないか
- `--check`の`hermes_home`と`python_executable`が普段のHermes環境と一致しているか
